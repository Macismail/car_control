...
public class CarServer {
    ...
    private void start() throws Exception {
        server = ServerBuilder.forPort(port)
                ...
                .build()
                .start();
        JmDNSRegistrationHelper helper = new JmDNSRegistrationHelper("Cars", "_car._udp.local.", "", port);
    ...
    private class CarImpl extends CarGrpc.CarImplBase {
        ...
        class RemindTask extends TimerTask {
            ...
            public void run() {
                if (percent < 100) {
                    percent += 10;
                    WindowsStatus status = WindowsStatus.newBuilder().setPercentageHeated(percent).build();
                    o.onNext(status);
                } else {
                    o.onCompleted();
                    this.cancel();
